...
public class FlutterFirebaseMessagingService extends FirebaseMessagingService {
  ...
  private static List<RemoteMessage> backgroundMessageQueue = Collections
      .synchronizedList(new LinkedList<RemoteMessage>());
 ...
 Log.i(TAG, "ExceptonCreate ck");
    // If background isolate is not running start it.
     Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
      intent.putExtra(EXTRA_REMOTE_MESSAGE, "owfejgwohgwoeghweiohgwoeh");
  ...
  public void onMessageReceived(final RemoteMessage remoteMessage) {
    ...
    if (isApplicationForeground(this)) {
      Log.i(TAG, "isApplicationForeground");

      Intent intent = new Intent(ACTION_REMOTE_MESSAGE);
      intent.putExtra(EXTRA_REMOTE_MESSAGE, remoteMessage);
      LocalBroadcastManager.getInstance(this).sendBroadcast(intent);
    } else {
      // If background isolate is not running yet, put message in queue and it will be
      // handled
   ...
   *              instance. This token is the same as the one retrieved by
   *              getInstanceId().
   */
  @Override
  ...
  /**
   * Setup the background isolate that would allow background messages to be
   * handled on the Dart side. Called either by the plugin when the app is
   * starting up or when the app receives a message while it is inactive.
   *
  ...
  /**
   * Acknowledge that background message handling on the Dart side is ready. This
   * is called by the Dart side once all background initialization is complete via
  ...
  /**
   * Set the method channel that is used for handling background messages. This
   ...
   * need to be handled on the Dart side the handler must be retrieved in the
   * background isolate to allow processing of the incoming message. This method
   * is called by the Dart side via `FcmDartService#start`.
   *
   * @param context Registrar context.
   * @param handle  Handle representing the Dart side method that will handle
   *                background messages.
   */
  ...
  /**
   * Set the background message setup handle for future use. The Dart side of this
   * plugin has a method that sets up the background method channel. When ready to
   * setup the background channel the Dart side needs to be able to retrieve the
   * setup method. This method is called by the Dart side via
   * `FcmDartService#start`.
   *
   * @param context               Registrar context.
   * @param setupBackgroundHandle Handle representing the dart side method that
   *                              will setup the background method channel.
   */
  ...
  /**
   * Retrieve the background message handle. When a background message is received
   * and must be processed on the dart side the handle representing the Dart side
   * handle is retrieved so the appropriate method can be called to process the
   * message on the Dart side. This method is called by
   * FlutterFirebaseMessagingServcie either when a new background message is
   * received or if background messages were queued up while background message
   * handling was being setup.
   *
   * @param context Application context.
   * @return Dart side background message handle.
   */
  public static Long getBackgroundMessageHandle(Context context) {
    return context.getSharedPreferences(SHARED_PREFERENCES_KEY, 0).getLong(BACKGROUND_MESSAGE_CALLBACK_HANDLE_KEY, 0);
  }

  /**
   * Process the incoming message in the background isolate. This method is called
   * only after background method channel is setup, it is called by
   * FlutterFirebaseMessagingServcie either when a new background message is
   * received or after background method channel setup for queued messages
   ...
   * @param latch         If set will count down when the Dart side message
   *                      processing is complete. Allowing any waiting threads to
   *                      continue.
   */
  ...
  /**
   * Identify if the application is currently in a state where user interaction is
   * possible. This method is only called by FlutterFirebaseMessagingService when
   * a message is received to determine how the incoming message should be
   * handled.
   *
   * @param context FlutterFirebaseMessagingService context.
   * @return True if the application is currently in a state where user
   *         interaction is possible, false otherwise.
   */
